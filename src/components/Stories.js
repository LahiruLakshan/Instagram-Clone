...
const Stories = () => {
  ...
  return (
      <ul className="w-full md:w-3/4 m-2 lg:w-1/2 flex justify-between items-start mb-8 space-x-3 overflow-x-scroll stories bg-white p-4 rounded drop-shadow-xl">
        {users.map((user) => (
          <li className="flex flex-none flex-col items-center space-y-1" key={user.id}>
            <div className={`p-1 rounded-full ${user.isUpdate ? 'bg-gradient-to-tr from-yellow-400 to-fuchsia-600' : 'border border-gray-300 bg-white border-1'}`}>
              <a href="#" className="block bg-white p-1 rounded-full relative">
                <img src={user.image_url} alt={user.name} className="w-16 h-16 rounded-full object-cover" />
              </a>
            </div>
            <a href="#" className="text-xs text-slate-800">{user.username}</a>
          </li>
        ))}
      </ul>
  );
